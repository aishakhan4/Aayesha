...
<div class='alert alert-danger' style="display: none;"><strong>Error !</strong> Student Roll Missing !</div>
        ...
        <?php
			...
			$query = "SELECT * FROM emppersonaldetail LIMIT " . $page_first_result . ',' . $results_per_page;  
			
			$rs=readrecord($query);

			?>
				
					<a class="btn btn-info float-right" href="empdate_view.php"><h1>View All</h1></a>
				</h2>
			</div>
				...
				<form action="" method="post">
					<table class="table table-striped">
						<tr>
							...
							<th width="25%"><h1>Employee ID<h1></th>
						...
						<td><h1><?php echo $i;?></h1></td>
							<td><h1><?php echo $row['full_name'];?></h1></td>
							<td><h1><?php echo $row['id'];?></h1></td>
							<td><h1>
								<input type="radio" name="attend[<?php echo $row['id'];?>]" value="present">P
								<input type="radio" name="attend[<?php echo $row['id'];?>]" value="absent">A
						</h1>	</td>
						</tr>
<?php
$i++;
        ...
        <?php
				 ...
				 if($page>=2){   
					echo " <li class='page-item'> <a  class='page-link' style='border:none;' href='takeempattend.php?page=".($page-1)."'> <h1> Prev </h1></a></li>";   
				}
				for($i = 1; $i<= $number_of_page; $i++) {  
        echo '<li class="page-item"> <a class="page-link" href = "takeempattend.php?page=' . $i . '"><h1 style=>' . $i . ' </h1></a></li>';  
	}
	
	if($page<$number_of_page){ 
  
		echo "<li class='page-item' style='border:none;'><a  class='page-link' style='border:none;' href='takeempattend.php?page=".($page+1)."'> <h1> Next </h1></a></li>";   
	}   
	echo "</ul>"
