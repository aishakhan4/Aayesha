...
<div class='alert alert-danger' style="display: none;"><strong>Error !</strong> Employee Id Missing !</div>
		<div class="card">
			<div class="card-header">
				<h2>
					<a class="btn btn-info float-right" href="empdate_view.php"><h1>Back</h1></a>
				</h2>
			</div>
			<?php 
			$date=$_GET['att_time'];
...
if(isset($_POST["submit"]))
{
 ...
 $qry="update emp_attendance set attend='$atn_value', att_time='$date' where id=$atn_key";
 $rs=executequery($qry);
 if($rs=="success")
 {
 ...
 <?php
   ...
   $qry="SELECT * FROM emp_attendance where att_time='$date'";
   $rs=readrecord($qry);
   ...
   $query="SELECT * FROM emp_attendance where att_time='$date' LIMIT " . $page_first_result . ',' . $results_per_page;  
				...
				<form action="" method="post">
					<table class="table table-striped">
						<tr>
							...
							<th width="25%"><h1>Employee Name</h1></th>
...
<?php 
$id=$row['id'];
$sql="SELECT full_name FROM emppersonaldetail WHERE id=$id";
$r=readrecord($sql);
$get_name=mysqli_fetch_array($r);
?>
<td><h1><?php echo $get_name['full_name'];?></h1></td>
	<td><h1><?php echo $row['id'];?></h1></td>
	<td><h1>
		<input type="radio" name="attend[<?php echo $row['id'];?>]"  value="present" <?php if($row['attend'] == "present") {echo "checked";} ?>>P
		<input type="radio" name="attend[<?php echo $row['id'];?>]" value="absent" <?php if($row['attend'] == "absent") {echo "checked";} ?>>A
</h1>	</td>
</tr>
<?php
$i++;
						...
						<tr>
							<td colspan="4" class="text-center">
								<input type="submit" name="submit" class="btn btn-primary px-5" value="Update" style="font-size=20px; height:35px;font-size:15px;">
							</td>
						</tr>
					</table>
				</form>
        <?php
				 ...
				 if($page>=2){   
					echo " <li class='page-item' style='border:none;'> <a  class='page-link' style='border:none;' href='emp_view.php?page=".($page-1)."'> <h1> Prev </h1></a></li>";   
				}
				for($i = 1; $i<= $number_of_page; $i++) {  
        echo '<li class="page-item"> <a class="page-link" href = "emp_view.php?page=' . $i . '"><h1 style=>' . $i . ' </h1></a></li>';  
	}
	
	if($page<$number_of_page){ 
  
		echo "<li class='page-item' style='border:none;'><a  class='page-link' style='border:none;' href='emp_view.php?page=".($page+1)."'> <h1> Next </h1></a></li>";   
	}   
	echo "</ul>"
